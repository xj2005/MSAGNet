...
class BaseBlock(MessagePassing):
    ...
    def aggregate_nodes(self, edge_features, edge_index, size, **kwargs):
        ...
        # 添加注意力机制
        # attention = torch.nn.functional.softmax(edge_features, dim=0)
        # updated_features = attention * edge_features
        # aggregated_features = updated_features.sum(dim=0)
        
        # node_features = self.aggregate(aggregated_features, **aggr_kwargs)
